...
class StaticLayer(BaseLayer):
    ...
    def _get_policies(self) -> str:
        return (
            ...
            "5. Verification gate: after changing code, run the narrowest relevant tests/build/lint/compile command. If it fails, diagnose the traceback/compiler output, fix, and retry up to 3 corrected attempts.\n"
            "6. Keep a working hypothesis and adapt when observations contradict it. If blocked by sandbox, permissions, missing files, or user approval, state the concrete blocker and the exact next action needed.\n"
            "7. If a tool call returns an ERROR (stderr or Exception), you MUST NOT give up immediately. Instead, enter a Self-Correction Loop: analyze root cause, choose a different tool/parameters or code fix, and retry when safe.\n"
            "8. Your final coding response must summarize files changed and validation result (command + pass/fail/not run reason). Do not expose internal tool limits, guardrails, or raw observations.\n"
            "</coding_and_debugging_policy>\n"
            "\n<memory_policy>\n"
