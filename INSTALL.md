...
- **安装卡在 `Preparing metadata (pyproject.toml)` 很久**
  这通常是 pip 在构建隔离环境、解析依赖或下载较重依赖。新版安装脚本会预装 `hatchling` 并使用 `--no-build-isolation` 安装核心包，默认安装也不再包含 `lancedb` / `pyarrow` / `tantivy` / `sentence-transformers` 等 RAG 重依赖。若需要语义记忆，再单独运行：
  ```bash
  pip install -e ".[rag]"
  ```
