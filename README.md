...
| 🧠 核心智能 | ✅ | **支持 SOUL.md & AGENTS.md**，显式 ReAct/CoT 推理循环 |
| 🤝 协作能力 | ✅ | **多 Agent 协作**，支持通过 `invoke_sub_agent` 委派任务 |
...
PyClaw v0.7.0 引入了深度推理与协作能力，使其能够处理更加复杂的任务：

### 1. 指令驱动架构
- **SOUL.md**: 全局人格定义。存放在 `~/.config/pyclaw/SOUL.md`，定义 Agent 的核心价值观、语气和行为准则。
- **AGENTS.md**: 项目级上下文。存放于项目根目录，为 Agent 提供当前工程的背景信息、编码规范和任务目标。

### 2. 显式推理循环 (ReAct + CoT)
Agent 在执行任何动作前都会进行“思考”与“规划”。
- **<thought>**: 记录 Agent 的内部思维链。
- **<plan>**: 为复杂任务生成多步执行计划。
这些思考过程会实时打印在控制台（或日志中），增加决策透明度。

### 3. 多 Agent 协作
通过 `invoke_sub_agent` 工具，主 Agent 可以将子任务委派给拥有独立会话空间的子 Agent。
- **上下文隔离**：子 Agent 处理细节，主 Agent 负责调度，有效避免长对话导致的 Token 溢出。
- **专业分工**：可以为子 Agent 指定专业领域（如 Python 专家、数据分析师）。
